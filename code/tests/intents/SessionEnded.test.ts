import { anyString, anything, capture, deepEqual, verify } from 'ts-mockito';
...
describe('Session ended request handler', () => {
  testCanHandle({
    ...
    testName: 'can handle any SessionEndedRequest',
  });

  testInAllLocales('clears session attributes')(async (locale) => {
    const mocks = await mockHandlerInput({ locale });

    SessionEndedRequestHandler.handle(mocks.instanceHandlerInput);

    verify(mocks.mockedAttributesManager.setSessionAttributes(deepEqual({}))).once();
  });

  testInAllLocales('clears dynamic entities')(async (locale) => {
    const mocks = await mockHandlerInput({ locale });
  ...
  testInAllLocales('does not speak nor reprompt')(async (locale) => {
    const mocks = await mockHandlerInput({ locale });
