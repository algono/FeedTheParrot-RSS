...
import { PAUSE_BETWEEN_FIELDS_CARD, PAUSE_BETWEEN_FIELDS } from '../util/constants';
...
export function getItems(feed: Feed, defaultLocale: string, options?: GetItemsOptions): Promise<FeedItem[]> {
    return new Promise<FeedItem[]>((resolve, reject) => {
        ...
        feedparser.on('readable', async function () {
            ...
            if (locale !== defaultLocale) {
                langFormatter = `<voice name="${t('DEFAULT_VOICE')}"><lang xml:lang="${t('DEFAULT_VOICE_LOCALE')}">%s</lang></voice>`;
            }
            ...
            function clean(text: string): string {
                ...
                cleanedText = cleanedText.replace(/[&]/g, ampersandReplacement).replace(/[<>]/g, '');
    ...
    */
    ...
    if (feed.readFields) {
        ...
        feed.readFields.forEach(field => {
            ...
            if (field.truncateAt) {
                console.log(`Field "${field.name}" truncated at ${field.truncateAt} characters`);
